...
const Items = ({product}) => {
    ...
    const available_quantity=product.available_quantity
    const id=product.id
    const price=product.current_price[0].NGN[0]
     const productImage=product.photos.map(photo => photo.url);
     const imageUrl=`https://api.timbu.cloud/images/${productImage}`
     console.log(cartItems)
    // https://api.timbu.cloud/images/james-clothing/product_happy_time_972aa2_1.jpg
  ...
  return (
    <div className=' md:w-[863.88px] md:h-[180px] w-[327.41px] mb-6 border pr-4 md:pr-1 bg-white border-gray-200 rounded-[10px] md:mr-4 h-[84.76px] pt-[8.04px] pl-[8.04] pb-[16px] flex flex-row items-center justify-between'>
            ...
            <div className='w-[60.32px] h-[60.32px] flex flex-row md:w-[332.02px] md:h-[140px]'>
                <img src={"https://api.timbu.cloud/images/james-clothing/product_happy_time_972aa2_1.jpg"} 
                alt='item image'
        ...
        <div className='md:w-[421.85px] md:h-[48px] w-[89.75px] h-[60px] flex flex-col justify-between items-end md:flex-row-reverse md:items-center '>
            ...
            <span className='w-[46.21px] h-[18px] text-[11.21px] leading-[17.93px]  font-bold'>{product.price}</span>
